...
const dateRangePickerStyles = `
  ...
  .handwritten-datepicker .react-datepicker__day--range-start,
  .handwritten-datepicker .react-datepicker__day--range-end {
    background: #333333 !important;
    color: white !important;
    border-radius: 0 !important;
    font-weight: 700 !important;
  }
  .handwritten-datepicker .react-datepicker__day--in-range,
  .handwritten-datepicker .react-datepicker__day--in-selecting-range {
...
// ─── DateRangePickerInput (trigger button) ─────────────────────────────────────────

interface DateRangePickerInputProps extends React.HTMLAttributes<HTMLDivElement> {
  border?: 'rough' | 'normal';
  disabled?: boolean;
  value?: string;
}
...
>(({ value, onClick, border = 'rough', disabled = false }, ref) => {
  ...
  return (
    <div
      ...
      className={`relative flex min-w-[240px] cursor-pointer items-center gap-2 px-4 py-2 ${
        border === 'normal'
        ...
        <div ref={ref} className={`handwritten-datepicker ${className}`.trim()}>
          {inline ? (
            ...
            <ReactDatePicker
              ...
              customInput={
                <DateRangePickerInput border={border} disabled={disabled} />
              }
            />
          )}
        </div>
